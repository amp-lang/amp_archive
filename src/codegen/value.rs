...
pub fn compile_struct_comparison(
    ...
    let mut res = builder.ins().iconst(cranelift::prelude::types::I8, 0);
    ...
    for (i, field) in struct_decl.fields.iter().enumerate() {
        ...
        if field.ty.value.is_primitive() {
                ...
                .is_big(checker, codegen.pointer_type.bytes() as usize)
            {
                ...
                builder
                    .ins()
                    .icmp_imm(cranelift::prelude::IntCC::Equal, cmp, 0)
            } else {
                ...
                builder
                    .ins()
                    .icmp(cranelift::prelude::IntCC::Equal, left, right)
            };
    ...
    let value = match value {
        ...
        Value::LogEq(left, right) => {
            ...
            if ty.is_primitive() {
                let lhs = compile_value(checker, codegen, builder, left, vars, data, None).unwrap();
                let rhs =
                    compile_value(checker, codegen, builder, right, vars, data, None).unwrap();
                ...
                if ty.is_big(checker, codegen.pointer_type.bytes() as usize) {
                    ...
                    builder
                        .ins()
                        .icmp(cranelift::prelude::IntCC::Equal, lhs, rhs)
                }
            } else {
                ...
                {
                    let lhs =
                        compile_value(checker, codegen, builder, left, vars, data, None).unwrap();
                    let rhs =
                        compile_value(checker, codegen, builder, right, vars, data, None).unwrap();
                    (lhs, rhs)
                } else {
